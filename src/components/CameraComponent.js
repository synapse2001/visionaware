...
const CameraComponent = () => {
  ...
  const loadSettings = () => {
    ...
    if (savedSettings.username === undefined) {
      savedSettings.username = '';
    }
    if (savedSettings.usernameLock === undefined) {
      savedSettings.usernameLock = false;
    }
    ...
    return {
      ...
      usernameLock: savedSettings.usernameLock || false
    };
  };
  ...
  useEffect(() => {
    if (username == undefined || username === "" || usernameLock === false) {
      setShowSettingPopup(true);
    }
  }, [username]);
